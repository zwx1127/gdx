...
Use `gdx` as the Godot automation layer. Codex remains responsible for game design, project architecture, GDScript, scene/resource specs, test logic, and interpreting failures. `gdx` only applies those decisions to a real Godot 4.x project through JSON-emitting commands.

## Start Here

1. Locate `gdx` and Godot:
   - Prefer `gdx` on `PATH`.
   - If working inside the gdx repo and `gdx` is not on `PATH`, use `target/debug/gdx.exe` on Windows or `target/debug/gdx` elsewhere after `cargo build --workspace`.
...
3. Implement in normal project files:
   ...
   - Create/copy assets under `res://assets/...`.
...
- Read `references/gdx-cli.md` for command syntax and error handling.
