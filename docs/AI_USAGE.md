...
gdx is intended to be driven by agents through subprocess calls. Every command emits JSON. Failures are emitted to stderr as JSON and include log artifacts plus diagnostics when a Godot process was started.
...
gdx --project <project> script load-check
...
`script check-all` is a strict Godot parser check over `.gd` files. It catches runtime parser failures such as warnings treated as errors. Use `script load-check` only when you specifically need the older fast resource-load check.
...
gdx --project <project> input click-node --target /StartButton
gdx --project <project> input activate --target /StartButton
...
For UI or gameplay regression checks, prefer a single verify spec:
